...
def fetch_portfolio_prices(symbols):
    """Fast price fetch for portfolio symbols including pre/post market."""
            ...
            for sym in batch:
                try:
                    ...
                    if price > 0:
                        ...
                        if pre:
                            entry['prePrice'] = round(float(pre), 4)
                            entry['preChangePct'] = round(((float(pre) - price) / price) * 100, 2)
                        # After-hours
                        post = info.get('postMarketPrice')
                        if post:
                            entry['postPrice'] = round(float(post), 4)
                            entry['postChangePct'] = round(((float(post) - price) / price) * 100, 2)
                        result[sym] = entry
                        print(f"  {sym}: ${price:.2f}" + (f" pre:${pre:.2f}" if pre else "") + (f" post:${post:.2f}" if post else ""))
...
def fetch_watchlist_data(symbols):
    ...
    for sym in symbols:
        try:
            ...
            pre_change  = round(((float(pre_price)  - price) / price) * 100, 2) if pre_price  else None
            post_change = round(((float(post_price) - price) / price) * 100, 2) if post_price else None
            ...
            entry = {
                ...
                'name':           name,
                'sector':         sector,
                'prePrice':       round(float(pre_price),  4) if pre_price  else None,
                'postPrice':      round(float(post_price), 4) if post_price else None,
                'preChangePct':   pre_change,
                'postChangePct':  post_change,
                # Business Quality
