    ...
    list.forEach(sym => {
      ...
      if (d && d.price) {
        wlData[sym] = {
          ...
          sector:         d.sector         || '',
          // Pre/post market
          prePrice:       d.prePrice       ?? null,
          postPrice:      d.postPrice      ?? null,
          preChangePct:   d.preChangePct   ?? null,
          postChangePct:  d.postChangePct  ?? null,
          // Technical
          rsi:            d.rsi            ?? null,
          ma20:           d.ma20           || null,
          ma50:           d.ma50           || null,
...
function wlCardHtml(sym, d, buyZonePct, watchZonePct) {
  ...
  const sp = getSmartPrice(d.price, d.prePrice, d.preChangePct, d.postPrice, d.postChangePct);
  const spLabel = sp.label === 'PRE' ? 'PRE-MKT' : sp.label === 'POST' ? 'AFTER-HRS' : 'PRICE';
  const spBadge = sp.label !== 'REG' ? `<span style="font-size:9px;font-weight:700;background:${sp.color}15;color:${sp.color};padding:1px 4px;border-radius:4px;margin-left:3px">${spLabel}</span>` : '';
  const spChange = sp.changePct ? ` <span style="font-size:11px;color:${sp.changePct>0?'#2d7a2d':'#c0392b'}">${sp.changePct>0?'+':''}${sp.changePct.toFixed(2)}%</span>` : '';
  ...
  return `<div class="${cardClass}">
    ...
    <div class="wl-grid">
      <div class="lot-field"><span class="lot-field-label">${spLabel}${spBadge}</span><span class="lot-field-val">$${sp.price.toFixed(2)}${spChange}</span></div>
...
function combinedCardHtml(sym, d, buyZonePct, watchZonePct) {
  ...
  // Smart price for combined card
  const csp = getSmartPrice(d.price, d.prePrice, d.preChangePct, d.postPrice, d.postChangePct);
  const cspChange = csp.changePct ? ` <span style="font-size:11px;color:${csp.changePct>0?'#2d7a2d':'#c0392b'}">${csp.changePct>0?'+':''}${csp.changePct.toFixed(2)}%</span>` : '';
  ...
  return `<div class="wl-card zone-${zone}">
    ...
    <div class="lot-grid" style="margin-bottom:10px">
      <div class="lot-field"><span class="lot-field-label">Price</span><span class="lot-field-val">$${csp.price.toFixed(2)}${cspChange}</span></div>
