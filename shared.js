...
// ── Smart price helper ───────────────────────────────────────────────────────
// Returns the most relevant price + label based on current UAE time
function getSmartPrice(regularPrice, prePrice, preChangePct, postPrice, postChangePct) {
  const now = new Date();
  // UAE is UTC+4
  const uaeHour = (now.getUTCHours() + 4) % 24;
  const uaeMin  = now.getUTCMinutes();
  const uaeTime = uaeHour + uaeMin / 60;

  // US Eastern is UTC-4 (EDT) or UTC-5 (EST)
  // Pre-market:  4:00 AM – 9:30 AM ET  = 12:00 PM – 5:30 PM UAE
  // Regular:     9:30 AM – 4:00 PM ET  = 5:30 PM  – 12:00 AM UAE
  // After-hours: 4:00 PM – 8:00 PM ET  = 12:00 AM – 4:00 AM UAE

  // Pre-market window: 12:00 PM – 5:30 PM UAE
  if (uaeTime >= 12 && uaeTime < 17.5 && prePrice) {
    return { price: prePrice, changePct: preChangePct, label: 'PRE', color: '#a06000' };
  }
  // After-hours window: 12:00 AM – 4:00 AM UAE
  if ((uaeTime < 4) && postPrice) {
    return { price: postPrice, changePct: postChangePct, label: 'POST', color: '#1a5fa8' };
  }
  // Regular market
  return { price: regularPrice, changePct: null, label: 'REG', color: null };
}

let allRows = [], processedData = { lots: [], buyGroups: [] };
window.livePricesData = {};
let livePrices = {};
let priceTimer = null;
let currentTab = 'lots';
let zoneFilter = 'all';
...
function switchTab(tab) {
  ...
  if (['watchlist','signals','support'].includes(tab) && Object.keys(wlData).length === 0) {
    refreshWatchlist();
  } else if (tab === 'signals' && Object.keys(wlData).length > 0) {
    renderSignals();
  } else if (tab === 'support' && Object.keys(wlData).length > 0) {
    renderSupport();
  }
}
